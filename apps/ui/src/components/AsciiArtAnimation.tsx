...
type CiutatisSprite = (typeof CIUTATIS_SPRITES)[number];
...
function spriteSize(sprite: CiutatisSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CIUTATIS_SPRITES[(baseCol + baseRow) % CIUTATIS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CIUTATIS_SPRITES[Math.floor(Math.random() * CIUTATIS_SPRITES.length)]!;
