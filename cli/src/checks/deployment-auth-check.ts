...
export function deploymentAuthCheck(config: CiutatisConfig): CheckResult {
  ...
  if (mode === "local_trusted") {
    if (!isLoopbackHost(config.server.host)) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `ciutatis configure --section server` and set host to 127.0.0.1",
      };
    }
    return {
      name: "Deployment/auth mode",
  ...
  if (auth.baseUrlMode === "explicit" && !auth.publicBaseUrl) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `ciutatis configure --section server` and provide a base URL",
    };
  }
  ...
  if (exposure === "public") {
    if (auth.baseUrlMode !== "explicit" || !auth.publicBaseUrl) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `ciutatis configure --section server` and select public exposure",
      };
    }
    try {
      ...
      return {
        ...
        canRepair: false,
        repairHint: "Run `ciutatis configure --section server` and provide a valid URL",
      };
    }
  }
