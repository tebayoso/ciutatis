...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" ciutatis run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("ciutatis onboard")} once, then retry ${pc.cyan("ciutatis run")}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@ciutatis/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @ciutatis/server package
  try {
    const mod = await import("@ciutatis/server");
    return await startServerFromModule(mod, "@ciutatis/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@ciutatis/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Ciutatis server entrypoint.\n` +
          `Tried: ${devEntry}, @ciutatis/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
