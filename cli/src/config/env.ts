...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Ciutatis environment variables",
    "# Generated by Ciutatis CLI commands",
...
export function resolveCiutatisEnvFile(configPath?: string): string {
...
export function loadCiutatisEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeCiutatisEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readCiutatisEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeCiutatisEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeCiutatisEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readCiutatisEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeCiutatisEnvEntries(next, filePath);
  return next;
}
