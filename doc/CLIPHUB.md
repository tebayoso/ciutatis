...
ClipHub is the public registry where people share, discover, and download Ciutatis company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Ciutatis what a package registry is to a programming language. Ciutatis already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Ciutatis — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Ciutatis instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Ciutatis spec. It contains:
...
```
Downloads the template and creates a new company in your local Ciutatis instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Ciutatis, export your company as a template, then publish:
...
ClipHub is a **separate service** from Ciutatis itself. Ciutatis is self-hosted; ClipHub is a hosted registry that Ciutatis instances talk to.
...
| **Ciutatis CLI** | `paperclipai install`, `paperclipai publish`, `paperclipai cliphub sync` — built into Ciutatis |
| **Ciutatis UI** | "Browse ClipHub" panel in the Ciutatis web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Ciutatis) |
| Backend | TypeScript + Hono (consistent with Ciutatis) |
| Database | PostgreSQL |
...
5. Ciutatis creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Ciutatis until it works well
...
## Relationship to Ciutatis

ClipHub is **not required** to use Ciutatis. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Ciutatis what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in Ciutatis web UI
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Ciutatis's own philosophy)
