...
3. In Ciutatis UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Ciutatis UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Ciutatis invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Ciutatis with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Ciutatis UI that new issue exists.
