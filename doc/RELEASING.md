...
Maintainer runbook for shipping Ciutatis across npm, GitHub, and the website-facing changelog surface.
...
Ciutatis uses calendar versions that still fit semver syntax:
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Ciutatis. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
