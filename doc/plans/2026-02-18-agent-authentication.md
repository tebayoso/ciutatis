...
Agents need API keys to authenticate with Ciutatis. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Ciutatis
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Ciutatis generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Ciutatis CLI
...
managed by the Ciutatis server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Ciutatis.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Ciutatis SKILL.md (or a link to it)
   - What information Ciutatis needs from the agent (e.g. webhook URL, adapter
...
5. Ciutatis stores the pending registration.
...
7. On approval, Ciutatis provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Ciutatis issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How Ciutatis should send you heartbeats",
...
1. Generate a Ciutatis invite link for the company.
...
4. A Ciutatis company member approves the new agent.
5. Ciutatis begins sending heartbeats to the OpenClaw webhook endpoint.
