...
This document defines the V1 design for a Ciutatis adapter that integrates with
...
4. synthesized stdout events for Ciutatis UI/CLI
...
3. **Agent callback auth to Ciutatis** uses a bootstrap exchange flow (no long-lived Ciutatis key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Ciutatis endpoints, not full SKILL.md prompt injection.
...
## Ciutatis Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived Ciutatis credentials in prompt text.

Flow:

1. Before launch/follow-up, Ciutatis mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Ciutatis validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Ciutatis API calls.
...
1. Prompt includes a compact instruction to fetch skills from Ciutatis.
...
4. Route by Cursor `agentId` to active Ciutatis run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from Ciutatis skill endpoints |
| Ciutatis auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Ciutatis context.
3. Add richer PR metadata surfacing in Ciutatis UI.
