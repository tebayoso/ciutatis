...
`PAP-447` asks how Ciutatis should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Ciutatis instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Ciutatis:

- some operators use Ciutatis to manage Ciutatis and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Ciutatis
- Claude and Codex expose different built-in affordances, so Ciutatis should not overfit to one tool

## Core Product Decision

Ciutatis should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Ciutatis
...
- Ciutatis should not assume that every adapter can see or use a host filesystem path directly
...
If Ciutatis makes worktrees agent-first, it will blur:
...
- Ciutatis core resolves and records execution workspace state
...
That gives Ciutatis room to support:
...
- Ciutatis handles workspace runtime services under the hood using project/workspace policy
...
Once Ciutatis is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Ciutatis should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Ciutatis cannot infer
...
- when issue moves to done, should Ciutatis auto-commit?
- should it auto-push?
...
Ciutatis already has the right foundation for a project-first model.
...
For server-spawned local adapters, Ciutatis already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Ciutatis should mirror directly

Implication:

- for `codex_local`, Ciutatis should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Ciutatis should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Ciutatis should separate:
...
Ciutatis should be able to express intentions such as:
...
These are not specific to Ciutatis. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a Ciutatis-specific `devServer` flag.
...
Ciutatis should distinguish between:
...
Ciutatis should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Ciutatis should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Ciutatis
...
For runtime services specifically, Ciutatis should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Ciutatis-only dev-server flags
...
- same embedded DB / Ciutatis instance can be reused through env/config injection when appropriate
...
- Ciutatis can decide whether to reuse or start a fresh service deterministically
...
3. Test against Ciutatis-on-Ciutatis workflows first.
...
7. Workspace runtime services are modeled generically, not as Ciutatis-specific dev-server toggles.
