...
Ciutatis already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Ciutatis should implement next.

## Product Goals

Ciutatis should let operators:
...
For the next implementation pass, Ciutatis should enforce these defaults:
...
Ciutatis should separate subscription-included usage from billed spend:
...
Ciutatis should have two threshold classes:
...
Ciutatis must know that a project is budget-paused and enforce it.
...
Ciutatis must also block execution before new work starts.
