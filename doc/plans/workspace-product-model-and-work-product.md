...
Ciutatis needs to support two very different but equally valid ways of working:
...
Today, Ciutatis already has the beginnings of this model:
...
- making Ciutatis feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Ciutatis users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Ciutatis deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Ciutatis into a full code review product
...
This object must be recorded explicitly so that Ciutatis can:
...
Ciutatis should treat PRs as a type of work product linked back to:
...
Ciutatis already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Ciutatis must be a first-class deployment mode

Ciutatis cannot assume that it is running on the same machine as the code.

In cloud deployments, Ciutatis may:
...
- `Runtime service`: a process or service Ciutatis owns or tracks for a workspace
...
It also lets cloud-hosted Ciutatis deployments point at codebases and remotes without pretending the Ciutatis host has direct filesystem access.
...
Without an explicit `execution workspace` record, Ciutatis has nowhere stable to attach:
...
Ciutatis needs a single place to show:
...
- Ciutatis-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Ciutatis owning the execution host
...
## 1. Ciutatis host and execution host must be decoupled
...
without the Ciutatis host itself running local git or local preview processes.
...
Ciutatis should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Ciutatis.
...
Ciutatis should keep workspace automation centered on a small set of hooks:
...
Ciutatis should provide the equivalent visibility in the project `Code` surface:
...
Ciutatis should treat execution workspaces as the natural home for:
...
Ciutatis should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Ciutatis should continue treating cleanup policy as part of the core workspace model:
...
Ciutatis should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Ciutatis host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Ciutatis should borrow Worktrunk's good execution patterns while keeping the broader Ciutatis model:
...
That keeps Ciutatis focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Ciutatis deployments remain first-class

That is a better fit for Ciutatis than either extreme:
