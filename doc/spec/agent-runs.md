...
This spec defines how Ciutatis actually runs agents while staying runtime-agnostic.
...
1. Ciutatis is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Ciutatis manages lifecycle, and on exit Ciutatis parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Ciutatis skill text block)
