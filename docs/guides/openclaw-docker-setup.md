...
How to get OpenClaw running in a Docker container for local development and testing the Ciutatis OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Ciutatis includes an end-to-end join smoke harness:
...
- probes and prints a Ciutatis host URL that is reachable from inside OpenClaw Docker
...
If your Ciutatis deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Ciutatis server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Ciutatis URL (typically `http://host.docker.internal:3100`).
