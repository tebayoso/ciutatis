# ClipHub: Marketplace for Ciutatis Team Configurations

> The "app store" for whole-company AI teams — pre-built Ciutatis configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Ciutatis-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Ciutatis company (agents, projects, governance) |
...
A complete Ciutatis company configuration:
...
Single-agent configurations designed to plug into a Ciutatis org:
...
Portable skill files that any Ciutatis agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Ciutatis's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Ciutatis version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Ciutatis company
...
| `POST` | `/api/listings/:id/install` | Install to Ciutatis company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Ciutatis company (or create new)
  → ClipHub API calls Ciutatis API to:
  ...
  → Redirect to Ciutatis dashboard with new team running
```
...
### 5.3 Creator: Export from Ciutatis → Publish

```
Running Ciutatis company → "Export as Blueprint" (CLI or UI)
  → Ciutatis exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Ciutatis brand blue/purple)
...
| Install flow | One-click deploy to Ciutatis company |
...
{
  "targetCompanyId": "uuid",         // Existing Ciutatis company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Ciutatis
- **Backend**: Node.js API (or extend Ciutatis server)
- **Database**: Postgres (can share Ciutatis's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Ciutatis auth (or OAuth2)

### 9.2 Integration with Ciutatis

ClipHub can be:
- **Option A**: A separate app that calls Ciutatis's API to install blueprints
- **Option B**: A built-in section of the Ciutatis UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Ciutatis UI and API.
...
- [ ] Install flow: blueprint → Ciutatis company
...
- [ ] "Export from Ciutatis" CLI command
