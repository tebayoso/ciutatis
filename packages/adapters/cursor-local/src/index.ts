...
Use when:
- You want Ciutatis to run Cursor Agent CLI locally as the agent runtime
...
- Ciutatis auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- Ciutatis auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
