...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@ciutatis/adapter-utils";
import {
  asString,
  asNumber,
  asStringArray,
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await listCiutatisSkillEntries(__moduleDir));
