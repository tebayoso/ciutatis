...
import type { Goal, Issue } from "@ciutatis/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Ciutatis APIs.",
...
const plugin: CiutatisPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
