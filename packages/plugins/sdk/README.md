...
Official TypeScript SDK for Ciutatis plugin authors.
...
- Plugin UI bundles run as same-origin JavaScript inside the main Ciutatis app. They can call ordinary Ciutatis HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Ciutatis instance at runtime.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Ciutatis information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
