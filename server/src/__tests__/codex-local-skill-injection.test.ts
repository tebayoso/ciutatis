...
import { ensureCodexSkillsInjected } from "@ciutatis/adapter-codex-local/server";

async function makeTempDir(prefix: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), prefix));
}

async function createCiutatisRepoSkill(root: string, skillName: string) {
...
describe("codex local adapter skill injection", () => {
  ...
  it("repairs a Codex Ciutatis skill symlink that still points at another live checkout", async () => {
    ...
    await createCiutatisRepoSkill(currentRepo, "paperclip");
    await createCiutatisRepoSkill(oldRepo, "paperclip");
  ...
  it("preserves a custom Codex skill symlink outside Ciutatis repo checkouts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
