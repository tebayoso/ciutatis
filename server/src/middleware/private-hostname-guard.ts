...
function blockedHostnameMessage(hostname: string): string {
  return (
    ...
    `If you want to allow this hostname, please run pnpm ciutatis allowed-hostname ${hostname}`
  );
}
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. If you want to allow a hostname, run pnpm ciutatis allowed-hostname <host>.";
      if (wantsJson) {
