...
/** Resolve the Ciutatis repo skills directory (built-in / managed skills). */
function resolveCiutatisSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const ciutatisSkillsDir = resolveCiutatisSkillsDir();

  // Build set of Ciutatis-managed skill names
  const ciutatisSkillNames = new Set<string>();
  if (ciutatisSkillsDir) {
    try {
      for (const entry of fs.readdirSync(ciutatisSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) ciutatisSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isPaperclipManaged: ciutatisSkillNames.has(entry.name),
      });
    }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Ciutatis host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Ciutatis is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm ciutatis allowed-hostname ${apiHost}`
    });
  }
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Ciutatis hostname is reachable and allowed via `pnpm ciutatis allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Ciutatis API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Ciutatis generates and persists one during join so pairing approvals are stable.
    ...
    The board approves the join request in Ciutatis before key claim is allowed.
    ...
    ## Step 4: Install Ciutatis skill in OpenClaw
    ...
    ${
      ...
      "Ensure Ciutatis is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    ...
    appendBlock(`
      ...
      - then restart Ciutatis and retry onboarding.
    `);
  }
