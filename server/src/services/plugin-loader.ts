...
/**
 * Naming convention for npm-published Ciutatis plugins.
 * Packages matching this pattern are considered Ciutatis plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
...
export interface DiscoveredPlugin {
  ...
  manifest: CiutatisPluginManifestV1 | null;
}
...
function getDeclaredPageRoutePaths(manifest: CiutatisPluginManifestV1): string[] {
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: CiutatisPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Ciutatis plugins installed as npm packages in the current
   ...
   *
   * Returns null if the package is not a Ciutatis plugin.
   * Throws if the package is a Ciutatis plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<CiutatisPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: CiutatisPluginManifestV1;
    newManifest: CiutatisPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Ciutatis plugin naming convention.
...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @ciutatis/plugin-*
  if (name.includes("/")) {
...
function getMinimumHostVersion(manifest: CiutatisPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumCiutatisVersion;
}
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: CiutatisPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as CiutatisPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Ciutatis plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<CiutatisPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Ciutatis plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasCiutatisPlugin = "paperclipPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasCiutatisPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<CiutatisPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @ciutatis/shared exports). Run those workers through
