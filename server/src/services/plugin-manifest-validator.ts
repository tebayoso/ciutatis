 ...
 */
import { pluginManifestV1Schema } from "@ciutatis/shared";
import type { CiutatisPluginManifestV1 } from "@ciutatis/shared";
import { PLUGIN_API_VERSION } from "@ciutatis/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): CiutatisPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as CiutatisPluginManifestV1,
        };
      }
